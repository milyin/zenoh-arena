    ...
    ) -> Result<Option<NodeStateInternal<E>>>
    {
        ...
        loop {
            ...
            tokio::select! {
                // Timeout elapsed
                () = &mut sleep => {
                    return Ok(Some(NodeStateInternal::Host(HostState {
                ...
                result = command_rx.recv_async() => match result {
                    ...
                    Ok(crate::node::NodeCommand::GameAction(action)) => {
                        ...
                        let _new_game_state = self.engine.process_action(action, node_id)?;
                        // Return the new state (game state is generated by step())
                        return Ok(Some(NodeStateInternal::Host(HostState {
                            connected_clients: self.connected_clients,
                            engine: self.engine,
                            liveliness_token: self.liveliness_token,
